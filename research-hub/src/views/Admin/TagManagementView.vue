...
function openEdit(tag: Tag): void { isEdit.value = true; editId.value = tag.id; form.name = tag.name; form.slug = tag.slug; form.color = tag.color; dialogVisible.value = true }

async function handleSave(): Promise<void> {
  if (!form.name.trim()) { ElMessage.warning('请输入标签名称'); return }
  try {
    if (isEdit.value && editId.value) { await adminApi.updateTag(editId.value, { name: form.name, slug: form.slug, color: form.color } as any); ElMessage.success('标签更新成功') }
    else { await adminApi.createTag({ name: form.name, slug: form.slug, color: form.color } as any); ElMessage.success('标签创建成功') }
    dialogVisible.value = false; await loadTags()
...
<template>
  <div class="admin-page">
    <div class="admin-header"><h2 class="admin-page-title">标签管理</h2><button class="btn-create" @click="openCreate">+ 新建标签</button></div>
    <div class="tag-grid" v-loading="loading">
      <div v-for="tag in tags" :key="tag.id" class="tag-card card--hoverable">
        <div class="tag-card-body">
          ...
          <div class="tag-info"><span class="tag-name">{{ tag.name }}</span><span class="tag-slug">{{ tag.slug }}</span></div>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑标签' : '新建标签'" width="420px" destroy-on-close>
      <el-form :model="form" label-position="top">
        <el-form-item label="标签名称" required><el-input v-model="form.name" placeholder="如：深度学习" /></el-form-item>
        <el-form-item label="Slug" required><el-input v-model="form.slug" placeholder="如：deep-learning" :disabled="isEdit" /></el-form-item>
        <el-form-item label="颜色"><div style="display:flex;align-items:center;gap:12px"><span class="color-preview" :style="{ background: form.color }" /><el-input v-model="form.color" placeholder="#4c6ef5" style="flex:1" /></div></el-form-item>
      </el-form>
...
.tag-name { font-size: 14px; font-weight: 500; } .tag-slug { font-size: 11px; color: #adb5bd; }
