...
async function loadCategories(): Promise<void> {
  loading.value = true
  try { const res = await resourceApi.getCategories(); categories.value = res.data } finally { loading.value = false }
}
...
function openEdit(cat: Category): void {
  isEdit.value = true; editId.value = cat.id
  form.name = cat.name; form.slug = cat.slug; form.description = cat.description
  dialogVisible.value = true
}
...
async function handleSave(): Promise<void> {
  ...
  try {
    if (isEdit.value && editId.value) {
      await adminApi.updateCategory(editId.value, { name: form.name, slug: form.slug, description: form.description } as any)
      ElMessage.success('分类更新成功')
    } else {
      await adminApi.createCategory({ name: form.name, slug: form.slug, description: form.description } as any)
...
async function handleDelete(id: number, name: string): Promise<void> {
  try {
    await ElMessageBox.confirm(`确定删除分类 "${name}"？`, '确认删除', { type: 'warning' })
...
<template>
  <div class="admin-page">
    <div class="admin-header">
      ...
      <button class="btn-create" @click="openCreate">+ 新建分类</button>
    </div>
      ...
      <div v-for="cat in categories" :key="cat.id" class="cat-item card--hoverable">
        <div class="cat-item-body">
          ...
          <div class="cat-actions">
            <button class="btn-sm" @click="openEdit(cat)">编辑</button>
            <button class="btn-sm danger" @click="handleDelete(cat.id, cat.name)">删除</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑分类' : '新建分类'" width="450px" destroy-on-close>
      <el-form :model="form" label-position="top">
        <el-form-item label="分类名称" required>
          <el-input v-model="form.name" placeholder="如：计算生物学" />
        </el-form-item>
        <el-form-item label="Slug" required>
          <el-input v-model="form.slug" placeholder="如：comp-bio" :disabled="isEdit" />
        </el-form-item>
        <el-form-item label="描述">
          <el-input v-model="form.description" type="textarea" :rows="2" placeholder="分类说明..." />
        </el-form-item>
      </el-form>
      <template #footer>
...
.cat-info { flex: 1; }
.cat-name { font-size: 14px; font-weight: 500; margin-bottom: 2px; }
.cat-slug { font-size: 11px; color: #adb5bd; }
.cat-desc { font-size: 12px; color: #6c757d; margin-top: 4px; }
