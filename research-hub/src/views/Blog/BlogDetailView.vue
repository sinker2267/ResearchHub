...
import { mockAddComment } from '@/mock'
...
async function load(): Promise<void> {
  ...
  try {
    ...
    await Promise.all([
      ...
      blogStore.fetchComments(id),
      blogStore.fetchRecommendations(id),
    ])
  } catch {
    error.value = true
  } finally {
...
async function handleLike(): Promise<void> {
  ...
  await blogStore.toggleLike(blogStore.currentPost.id)
}
      ...
      <div class="page-container--narrow">
        ...
        <article class="article">
          <header class="article-header">
            ...
            <div class="article-meta">
              <div class="meta-left">
                <span class="meta-author">{{ blogStore.currentPost.author.displayName }}</span>
                <span class="meta-sep">·</span>
                <span class="meta-time">{{ formatDate(blogStore.currentPost.publishedAt, 'full') }}</span>
              </div>
              <div class="meta-right">
                <span class="meta-stat"><el-icon :size="15"><View /></el-icon> {{ blogStore.currentPost.viewCount }}</span>
              </div>
            </div>
          </header>
          ...
          <div class="article-body">
            <MarkdownRenderer :content="blogStore.currentPost.content" />
          </div>

          <!-- Tags -->
          <div v-if="blogStore.currentPost.tags.length" class="article-tags">
            <span>标签：</span>
            <span v-for="tag in blogStore.currentPost.tags" :key="tag.id" class="tag-chip">{{ tag.name }}</span>
          </div>
          ...
          <div class="actions-bar">
            <button
              class="action-btn"
              :class="{ active: blogStore.currentPost.isLiked }"
              @click="handleLike"
            >
              <el-icon :size="16"><Star /></el-icon>
              <span>{{ blogStore.currentPost.isLiked ? '已点赞' : '点赞' }} ({{ blogStore.currentPost.likeCount }})</span>
            </button>
            <button
              class="action-btn"
              :class="{ active: blogStore.currentPost.isFavorited }"
              @click="handleFavorite"
            >
              {{ blogStore.currentPost.isFavorited ? '★ 已收藏' : '☆ 收藏' }}
            </button>
            <button class="action-btn">
              <el-icon :size="16"><Share /></el-icon> 分享
            </button>
            <button
              v-if="userStore.hasPermission(PERMISSIONS.BLOG_EDIT)"
              class="action-btn"
              @click="handleEdit"
            >
        ...
        <section class="comments-section">
          <h3 class="section-heading">
            ...
            评论 ({{ blogStore.currentPost.commentCount }})
          </h3>
          ...
          <EmptyState v-else title="暂无评论" description="成为第一个评论的人" />
        </section>
        ...
        <section v-if="blogStore.recommendations.length" class="recommendations-section">
          ...
          <div class="rec-grid">
            <BlogCard v-for="rec in blogStore.recommendations" :key="rec.id" :post="rec" />
          </div>
        </section>
      </div>
    </template>
...
.article-title {
  ...
  color: #212529; line-height: 1.4; margin-bottom: 16px;
}
...
.comments-section, .recommendations-section {
  margin-top: 32px;
}
