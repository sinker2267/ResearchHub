          ...
          <el-form label-position="top" size="default">
            ...
            <el-form-item label="紧急程度">
              <el-radio-group v-model="form.level">
                <el-radio-button v-for="lvl in levels" :key="lvl.value" :value="lvl.value">{{ lvl.label }}</el-radio-button>
              </el-radio-group>
            </el-form-item>
            <div class="form-actions">
              <button class="btn-cancel" @click="router.back()">取消</button>
              <button class="btn-submit" :disabled="saving" @click="handleSubmit">{{ saving ? '发布中...' : '发布公告' }}</button>
            </div>
          </el-form>
        </div>
      </div>
...
.btn-cancel { padding: 9px 22px; font-size: 13px; color: #6c757d; background: #fff; border: 1px solid #dee2e6; border-radius: 6px; cursor: pointer; &:hover { background: #f1f3f5; } }
